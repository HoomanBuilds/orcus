export const vaultAbi = [
  {
    inputs: [
      { internalType: "address", name: "_teeAgentAddress", type: "address" },
      { internalType: "address", name: "_jaineRouter", type: "address" },
    ],
    stateMutability: "nonpayable",
    type: "constructor",
  },
  {
    anonymous: false,
    inputs: [
      { indexed: true, internalType: "address", name: "user", type: "address" },
      { indexed: false, internalType: "uint256", name: "amount", type: "uint256" },
    ],
    name: "IntentSet",
    type: "event",
  },
  {
    anonymous: false,
    inputs: [
      { indexed: true, internalType: "address", name: "user", type: "address" },
      { indexed: false, internalType: "bytes32", name: "receiptHash", type: "bytes32" },
      { indexed: false, internalType: "bytes", name: "teeAttestation", type: "bytes" },
    ],
    name: "TradeExecuted",
    type: "event",
  },
  {
    inputs: [{ internalType: "address", name: "", type: "address" }],
    name: "balances",
    outputs: [{ internalType: "uint256", name: "", type: "uint256" }],
    stateMutability: "view",
    type: "function",
  },
  {
    inputs: [
      { internalType: "bytes", name: "_encryptedGoal", type: "bytes" },
      { internalType: "uint256", name: "_maxSlippage", type: "uint256" },
      { internalType: "uint256", name: "_stopLoss", type: "uint256" },
    ],
    name: "depositAndSetIntent",
    outputs: [],
    stateMutability: "payable",
    type: "function",
  },
  {
    inputs: [
      { internalType: "address", name: "user", type: "address" },
      { internalType: "bytes", name: "tradeData", type: "bytes" },
      { internalType: "bytes", name: "teeAttestation", type: "bytes" },
      { internalType: "bytes32", name: "storageReceiptHash", type: "bytes32" },
    ],
    name: "executeTradeWithProof",
    outputs: [],
    stateMutability: "nonpayable",
    type: "function",
  },
  {
    inputs: [{ internalType: "address", name: "", type: "address" }],
    name: "intents",
    outputs: [
      { internalType: "bytes", name: "encryptedGoal", type: "bytes" },
      { internalType: "uint256", name: "maxSlippage", type: "uint256" },
      { internalType: "uint256", name: "stopLoss", type: "uint256" },
      { internalType: "uint256", name: "depositAmount", type: "uint256" },
      { internalType: "bool", name: "active", type: "bool" },
    ],
    stateMutability: "view",
    type: "function",
  },
  {
    inputs: [],
    name: "jaineRouter",
    outputs: [{ internalType: "address", name: "", type: "address" }],
    stateMutability: "view",
    type: "function",
  },
  {
    inputs: [],
    name: "teeAgentAddress",
    outputs: [{ internalType: "address", name: "", type: "address" }],
    stateMutability: "view",
    type: "function",
  },
  {
    inputs: [],
    name: "withdraw",
    outputs: [],
    stateMutability: "nonpayable",
    type: "function",
  },
] as const;
