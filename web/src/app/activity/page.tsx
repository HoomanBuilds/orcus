...
export default function ActivityPage() {
  const { vm, rows, isLoading } = usePortfolio({ userScoped: false, publicFallback: true });
  const uniqueTraders = new Set(rows.map((e) => e.user)).size;
  const isEmpty = !isLoading && rows.length === 0;
  const scopeLabel = vm === "sui" ? "Sui testnet" : "5 EVM chains";
        ...
        <div className="max-w-6xl mx-auto flex flex-col gap-8">
          ...
          <div className="flex items-end justify-between">
            <div>
              ...
              <p className="mt-2 text-sm text-black/40 max-w-sm">Live execution feed across {scopeLabel}. Every trade verified on-chain.</p>
            </div>
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
            {[
              { label: "Total executions", value: isLoading ? "…" : rows.length.toString(), sub: "TradeExecuted events" },
              { label: "Unique traders", value: isLoading ? "…" : uniqueTraders.toString(), sub: "distinct addresses" },
              { label: "Scope", value: vm === "sui" ? "Sui" : "EVM ×5", sub: vm === "sui" ? "Move VM" : "galileo + 4 testnets" },
              { label: "Proof storage", value: "0G Storage", sub: "permanent receipts" },
            ].map((s) => (
          ...
          <div className="relative overflow-hidden rounded-2xl border border-black/[0.07] bg-white min-h-[130px] flex items-end">
            <img src="https://hebbkx1anhila5yf.public.blob.vercel-storage.com/executor-o1q6509qMLXMtpBIGo49vcgOu34sI1.png" alt="" aria-hidden="true"
              className="absolute inset-0 w-full h-full object-cover object-center"
              style={{ maskImage: "linear-gradient(to bottom, black 0%, black 40%, transparent 90%)", WebkitMaskImage: "linear-gradient(to bottom, black 0%, black 40%, transparent 90%)" }} />
          ...
          <div className="rounded-2xl border border-black/[0.07] bg-white overflow-hidden">
            <div className="grid px-6 py-3 border-b border-black/[0.05]" style={{ gridTemplateColumns: "1fr 1.5fr 2fr 64px 64px", gap: "1rem" }}>
              {["Chain", "Trader", "Receipt hash", "Tx", "Proof"].map((h) => (
                <span key={h} className="text-[10px] tracking-[0.14em] uppercase text-black/30" style={{ fontFamily: "var(--font-data)" }}>{h}</span>
              ))}
            </div>

            {isLoading && <div className="px-6 py-16 text-center"><p className="text-sm text-black/30">Fetching on-chain events…</p></div>}
            ...
            {isEmpty && (
              <div className="px-6 py-20 text-center flex flex-col items-center gap-3">
                ...
                <p className="text-xs text-black/20"><Link href="/strategy" className="underline">Set an intent</Link> to be the first</p>
              </div>
            )}
            ...
            {rows.map((ex, i) => {
              ...
              return (
                <div key={ex.txHash || i} className="group grid px-6 py-3.5 items-center hover:bg-black/[0.015] transition-colors"
                  style={{ gridTemplateColumns: "1fr 1.5fr 2fr 64px 64px", gap: "1rem", borderBottom: i < rows.length - 1 ? "1px solid rgba(0,0,0,0.04)" : "none" }}>
                  <ChainBadge chainKey={ex.chainKey} />
          ...
          <div className="flex items-center gap-6 pt-4 border-t border-black/[0.06]">
            {[{ label: "Dashboard", href: "/dashboard" }, { label: "Strategy Terminal", href: "/strategy" }, { label: "My History", href: "/history" }].map((l) => (
